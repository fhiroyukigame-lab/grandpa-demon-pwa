...
function stats(){const s={hp:120+state.level*8,atk:10+state.level*2,def:4+state.level,spd:50,crit:5,eva:3,lifesteal:0};Object.values(state.equipped).map(gear).forEach(g=>{if(!g)return;[g.base,g.bonus].forEach(src=>Object.entries(src||{}).forEach(([k,v])=>s[k]=(s[k]||0)+v))});return s}
...
function win(){if(!battle)return;const f=battle.floor,r=battle.e.reward;clearInterval(battleTimer);battleTimer=null;battle.e.currentHp=0;updateBattleUI();$('#enemyAvatar').classList.add('enemy-defeated');state.gold+=r;state.maxFloor=Math.max(state.maxFloor,f);state.level=1+Math.floor(state.maxFloor/3);if(f<30)state.selectedFloor=Math.min(30,f+1);log(`勝利！ ${r}Gを獲得！`);floatText($('#battleArea'),`💰 +${r} G`,'gold-float');save();$('#gold').textContent=state.gold;$('#maxFloor').textContent=state.maxFloor;$('#selectedFloor').textContent=state.selectedFloor;setTimeout(()=>{battle=null;render()},750)}function lose(){clearInterval(battleTimer);battleTimer=null;log('じいさんは力尽きた……。');battle=null;updateBattleUI()}function log(t){$('#battleLog').insertAdjacentHTML('beforeend',`<p>${t}</p>`);$('#battleLog').scrollTop=$('#battleLog').scrollHeight}
