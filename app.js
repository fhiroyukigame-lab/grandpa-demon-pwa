...
$('#newGameBtn').onclick=()=>{playSE();if(localStorage.getItem('grandpaDemonSave')&&!confirm('現在のセーブデータを消してニューゲームを始めますか？'))return;const chosen=(nameInputV53.value||'').trim()||randomGrandpaNameV53();localStorage.removeItem('grandpaDemonSave');state=migrate(null);state.playerName=chosen;save();setTimeout(()=>{enterGame();const equipBtn=document.querySelector('.tab[data-tab="equip"]');if(equipBtn)switchTab('equip',equipBtn)},180)};
$('#continueBtn').onclick=()=>{playSE();state=migrate(load());setTimeout(()=>{enterGame();const equipBtn=document.querySelector('.tab[data-tab="equip"]');if(equipBtn)switchTab('equip',equipBtn)},180)};
