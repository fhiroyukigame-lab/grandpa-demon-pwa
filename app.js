...
const $=s=>document.querySelector(s);const titleBgm=$('#titleBgm'),menuBgm=$('#menuBgm'),battleBgm=$('#battleBgm'),forgeBgm=$('#forgeBgm'),hitSe=$('#hitSe'),buttonSe=$('#buttonSe'),equipButtonSe=$('#equipButtonSe'),slotButtonSe=$('#slotButtonSe'),rouletteSe=$('#rouletteSe'),slotResultSe=$('#slotResultSe');
...
function playAudio(a){if(!a)return;try{a.pause();a.currentTime=0;a.play().catch(()=>{})}catch{}}function playSE(){playAudio(buttonSe)}function playEquipSE(){playAudio(equipButtonSe)}
...
function renderGrandpa(el){if(!el)return;el.innerHTML='';['body','left','right','head'].forEach(slot=>{const g=gear(state.equipped[slot]);if(!g)return;const sp=document.createElement('span');sp.className='gear-layer gear-'+(slot==='right'?'weapon':slot==='left'?'shield':'armor');sp.textContent=gearEmoji(slot,g.suffix);el.appendChild(sp)})}
function render(){const s=stats();$('#gold').textContent=state.gold;$('#level').textContent=state.level;$('#hpStat').textContent=s.hp;$('#atkStat').textContent=s.atk;$('#defStat').textContent=s.def;$('#spdStat').textContent=s.spd;$('#critStat').textContent=s.crit+'%';$('#evaStat').textContent=s.eva+'%';$('#maxFloor').textContent=state.maxFloor;$('#selectedFloor').textContent=state.selectedFloor;renderGrandpa($('#shopGrandpa'));renderGrandpa($('#playerAvatar'));renderEquip();renderForge();renderSkills();save()}
...
function renderForge(){const root=$('#forgeList');root.innerHTML='';const named=state.gears.filter(g=>g.locked);if(named.length<2){root.innerHTML='<p class="item-meta">合成には命名済み装備が2個以上必要です。</p>';return}named.forEach(target=>{const mats=named.filter(g=>g.id!==target.id);const div=document.createElement('div');div.className='gear-card';const cost=100+target.level*80;div.innerHTML=`<b>${target.name}</b><div class="item-meta">現在 +${target.level}｜${statSummary(target)}｜合成費用 ${cost}G</div><select class="forge-select">${mats.map(m=>`<option value="${m.id}">${m.name}（素材）</option>`).join('')}</select><div class="gear-actions"><button class="primary">選んだ装備を素材に合成</button></div>`;div.querySelector('button').onclick=()=>forge(target.id,div.querySelector('select').value,cost);root.appendChild(div)})}
...
function updateBattleUI(){const btns=[$('#skillPower'),$('#skillDodge'),$('#skillThrow')];btns.forEach((b,i)=>{const n=state.equippedSkills[i];b.textContent=n?`${n}${cooldowns[i]?` (${Math.ceil(cooldowns[i]/1000)})`:''}`:'スキル未装備';b.disabled=!battle||!n||cooldowns[i]>0});if(!battle)return;const p=battle.p,e=battle.e;$('#playerHpBar').style.width=Math.max(0,p.currentHp/p.maxHp*100)+'%';$('#enemyHpBar').style.width=Math.max(0,e.currentHp/e.maxHp*100)+'%';$('#playerHpText').textContent=`${Math.max(0,Math.round(p.currentHp))} / ${p.maxHp}`;$('#enemyHpText').textContent=`${Math.max(0,Math.round(e.currentHp))} / ${e.maxHp}`}
...
function switchTab(tab,btn){const doSwitch=()=>{document.querySelectorAll('.tab,.panel').forEach(x=>x.classList.remove('active'));btn.classList.add('active');$('#'+tab).classList.add('active');setHeroForge(tab==='forge');if(tab==='forge')playBgm(forgeBgm);else if(tab==='battle')playBgm(battleBgm);else playBgm(menuBgm)};if(tab==='forge'){const ov=$('#sceneTransition');ov.classList.remove('hidden');void ov.offsetWidth;ov.classList.add('show');setTimeout(()=>{doSwitch();ov.classList.add('fadeout');setTimeout(()=>{ov.classList.add('hidden');ov.classList.remove('show','fadeout')},500)},650)}else doSwitch()}
function enterGame(){playBgm(menuBgm);$('#titleScreen').classList.add('hidden');$('#gameApp').classList.remove('hidden');render()}function showTitle(){$('#gameApp').classList.add('hidden');$('#titleScreen').classList.remove('hidden');$('#continueBtn').disabled=!localStorage.getItem('grandpaDemonSave')}
let titleAudioUnlocked=false;$('#titleScreen').addEventListener('pointerdown',()=>{if(!titleAudioUnlocked){titleAudioUnlocked=true;playBgm(titleBgm)}},{once:true});
$('#newGameBtn').onclick=()=>{playSE();if(localStorage.getItem('grandpaDemonSave')&&!confirm('現在のセーブデータを消してニューゲームを始めますか？'))return;localStorage.removeItem('grandpaDemonSave');state=migrate(null);save();titleBgm.play().catch(()=>{});setTimeout(enterGame,180)};$('#continueBtn').onclick=()=>{playSE();state=migrate(load());titleBgm.play().catch(()=>{});setTimeout(enterGame,180)};
$('#gearGacha').onclick=gearGacha;$('#skillGacha').onclick=skillGacha;
document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>switchTab(b.dataset.tab,b));$('#minusFloor').onclick=()=>{state.selectedFloor=Math.max(1,state.selectedFloor-1);render()};$('#plusFloor').onclick=()=>{state.selectedFloor=Math.min(30,Math.max(state.maxFloor+1,1),state.selectedFloor+1);render()};$('#startBattle').onclick=startBattle;$('#skillPower').onclick=()=>useSkill(0);$('#skillDodge').onclick=()=>useSkill(1);$('#skillThrow').onclick=()=>useSkill(2);$('#resetSave').onclick=()=>{if(confirm('本当にセーブデータを初期化しますか？')){localStorage.removeItem('grandpaDemonSave');location.reload()}};
...
function render(){
  const s=stats();const set=(id,val)=>{const e=$(id);if(e)e.textContent=val};set('#gold',state.gold);set('#playerNameDisplay',state.playerName||'じいさん');set('#battlePlayerName',state.playerName||'じいさん');set('#hpStat',s.hp);set('#atkStat',s.atk);set('#defStat',s.def);set('#spdStat',s.spd);set('#critStat',s.crit+'%');set('#evaStat',s.eva+'%');set('#lifestealStat',Math.round((s.lifesteal||0)*100)+'%');set('#maxFloor',state.maxFloor);set('#selectedFloor',state.selectedFloor);renderGrandpa($('#shopGrandpa'));renderGrandpa($('#playerAvatar'));renderEquip();renderForge();renderSkills();bindHeroButtonsV52();save()
}
...
function switchTab(tab,btn){
  const doSwitch=()=>{document.querySelectorAll('.tab,.panel').forEach(x=>x.classList.remove('active'));btn.classList.add('active');$('#'+tab).classList.add('active');setHeroModeV53(tab==='forge'?'forge':tab==='gacha'?'shop':'normal');if(tab==='forge')playBgm(forgeBgm);else if(tab==='battle')playBgm(battleBgm);else playBgm(menuBgm)};
...
/* ===== Ver.5.3: name, no player level, shop/forge presentation ===== */
function randomGrandpaNameV53(){return GRANDPA_NAMES[Math.floor(Math.random()*GRANDPA_NAMES.length)]}
const nameInputV53=$('#grandpaNameInput');
if(nameInputV53&&!nameInputV53.value)nameInputV53.value=randomGrandpaNameV53();
if($('#randomNameBtn'))$('#randomNameBtn').onclick=()=>{playSE();nameInputV53.value=randomGrandpaNameV53()};
$('#newGameBtn').onclick=()=>{playSE();if(localStorage.getItem('grandpaDemonSave')&&!confirm('現在のセーブデータを消してニューゲームを始めますか？'))return;const chosen=(nameInputV53.value||'').trim()||randomGrandpaNameV53();localStorage.removeItem('grandpaDemonSave');state=migrate(null);state.playerName=chosen;save();titleBgm.play().catch(()=>{});setTimeout(()=>{enterGame();const equipBtn=document.querySelector('.tab[data-tab="equip"]');if(equipBtn)switchTab('equip',equipBtn)},180)};
$('#continueBtn').onclick=()=>{playSE();state=migrate(load());titleBgm.play().catch(()=>{});setTimeout(()=>{enterGame();const equipBtn=document.querySelector('.tab[data-tab="equip"]');if(equipBtn)switchTab('equip',equipBtn)},180)};
